...
public class Users {
    ...
    public void changeUserPassword(final int user_num, final int days_ago) {

        if (LocalDate.now().getDayOfMonth() - users[user_num].getRegistrationDate().getDayOfMonth() >= days_ago) {
            users[user_num].setPassword(utilityPasswordMaker.generatePassword());
        }
    }
